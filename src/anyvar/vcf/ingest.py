...
class VcfRegistrar(VcfAnnotator):
    ...
    def __init__(self, data_proxy: _DataProxy, **kwargs) -> None:  # noqa: D107
        ...
        if av is None:
            raise ValueError
        self.av: AnyVar = av
        self.vrs_object_registration_batcher = VrsObjectRegistrationBatcher(
            anyvar_instance=self.av
        )
    ...
    def on_vrs_object(
        ...
        **kwargs,
        ...
        self.vrs_object_registration_batcher.add_to_batch(vrs_object=vrs_allele)
        return vrs_allele
