...
def _register_variations(
    ...
    variations_to_store: list[types.VrsObject] = []
...
def register_vrs_variation(
    ...
    try:
        ...
        variation = types.recursive_identify(variation)
        av.put_objects([variation])
...
def get_object_by_id(
    ...
    av: AnyVar = request.app.state.anyvar
    vrs_object: types.VrsObject = _get_vrs_object(av, vrs_id)
...
def add_object_mapping(
    ...
    av: AnyVar = request.app.state.anyvar
    source_vrs_obj: types.VrsObject = _get_vrs_object(av, vrs_id)
    dest_vrs_id = mapping_request.dest_id
    dest_vrs_obj: types.VrsObject = _get_vrs_object(av, dest_vrs_id)
