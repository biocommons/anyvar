...
from anyvar.restapi.schema import TranslateFromResponse, TranslateToResponse
...
def translate_to(
    ...
    """Translate the specified VRS object to other identifiers"""
    av: AnyVar = request.app.state.anyvar
    translator: AlleleTranslator = av.translator.allele_tlr
    try:
        identifiers = {
            fmt: translator.translate_to(vrs_object, fmt)
            for fmt in translator.to_translators
        }
...
def translate_from(
    request: Request,
    fmt: Annotated[
        StrictStr, Path(..., description="Format for identifier (i.e. hgvs, gnomad)")
    ],
    identifier: Annotated[StrictStr, Path(..., description="Identifier")],
) -> TranslateFromResponse:
    """Translate the specified identifier to a VRS Object"""
    av: AnyVar = request.app.state.anyvar
    translator: AlleleTranslator = av.translator.allele_tlr
    try:
        vrs_object = translator.translate_from(identifier, fmt)
