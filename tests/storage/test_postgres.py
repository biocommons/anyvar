...
def test_create_schema(mocker):
    ...
    mock_eng.return_value.add_mock_stmt_sequence(
        MockStmtSequence()
        .add_stmt(
            f"SELECT EXISTS (SELECT 1 FROM pg_catalog.pg_tables WHERE tablename = '{vrs_object_table_name}')",
            None,
            [(False,)],
        )
        .add_stmt(
            f"CREATE TABLE {vrs_object_table_name} ( vrs_id TEXT PRIMARY KEY, vrs_object JSONB )",
            None,
            [("Table created",)],
        )
    )
...
def test_add_many_items(mocker):
    ...
    insert_statement = (
        "INSERT INTO vrs_objects2 SELECT * FROM tmp_table ON CONFLICT DO NOTHING"
    )
    ...
    mock_eng.return_value.add_mock_stmt_sequence(
        MockStmtSequence()
        .add_stmt(
            "SELECT EXISTS (SELECT 1 FROM pg_catalog.pg_tables WHERE tablename = 'vrs_objects2')",
            None,
            [(True,)],
        )
        # Batch 1
        ...
        .add_copy_from(
            "tmp_table",
            "\n".join(
                [f"{pair[0]}\t{pair[1].to_json()}" for pair in vrs_id_object_pairs[0:2]]
            ),
        )
        ...
        .add_copy_from(
            "tmp_table",
            "\n".join(
                [f"{pair[0]}\t{pair[1].to_json()}" for pair in vrs_id_object_pairs[2:4]]
            ),
        )
        ...
        .add_copy_from(
            "tmp_table",
            "\n".join(
                [f"{pair[0]}\t{pair[1].to_json()}" for pair in vrs_id_object_pairs[4:6]]
            ),
        )
        ...
        .add_copy_from(
            "tmp_table",
            "\n".join(
                [f"{pair[0]}\t{pair[1].to_json()}" for pair in vrs_id_object_pairs[6:8]]
            ),
        )
        .add_stmt(insert_statement, None, [(2,)], 5)
        .add_stmt(drop_statement, None, [("Table dropped",)])
        ...
        .add_copy_from(
            ...
            "\n".join(
                [
                    f"{pair[0]}\t{pair[1].to_json()}"
                    for pair in vrs_id_object_pairs[8:10]
                ]
            ),
        )
        ...
        .add_copy_from(
            ...
            "\n".join(
                [
                    f"{pair[0]}\t{pair[1].to_json()}"
                    for pair in vrs_id_object_pairs[10:11]
                ]
            ),
        )
    ...
    sf = PostgresObjectStore(
        "postgres://account/?param=value", 2, "vrs_objects2", 4, False
    )
