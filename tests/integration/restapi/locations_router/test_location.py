...
def test_location(restapi_client: TestClient, preloaded_alleles: dict):
    ...
    for allele in preloaded_alleles.values():
        vrs_id = allele["variation"]["location"]["id"]
        resp = restapi_client.get(f"/sequence_locations/{vrs_id}")
        assert resp.status_code == HTTPStatus.OK
        assert resp.json()["data"] == allele["variation"]["location"]

    # invalid ID
    bad_resp = restapi_client.get("/sequence_locations/not_a_real_location")
