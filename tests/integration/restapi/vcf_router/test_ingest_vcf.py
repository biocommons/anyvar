...
def test_handle_preannotated(
    restapi_client: TestClient, basic_annotated_vcf: io.BytesIO
):
    """Test that client gracefully handles a VCF that's already been annotated"""
    resp = restapi_client.put("/vcf", files={"vcf": ("test.vcf", basic_annotated_vcf)})
