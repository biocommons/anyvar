...
* **Configure test database** - unit and integration tests will set up a storage instance using the connection string defined by the environment variable ``ANYVAR_TEST_STORAGE_URI``, which defaults to ``"postgresql://postgres:postgres@localhost:5432/anyvar_test"``. Ensure that the database and role are available in the PostgreSQL instance, e.g. with the shell command ``psql -U postgres -C "CREATE DATABASE anyvar_test WITH OWNER postgres;"``
